...
export function PennyDropsAvatar() {
  const containerRef = useRef<HTMLDivElement>(null);
  const scriptRef = useRef<HTMLScriptElement | null>(null);
  const containerIdRef = useRef('pennydrops-avatar-container');

  useEffect(() => {
    const containerId = containerIdRef.current;

    console.log("[PennyDropsAvatar] init", { containerId, ts: new Date().toISOString() });
    ...
    script.setAttribute('data-name', 'pennydrops-agent');
